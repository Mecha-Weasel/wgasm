...
if ! [[ $COMPATIBLE_OS == true ]]; then
	MESSAGE="${ANSI_REDLT}ERROR:${ANSI_OFF}\n";
	MESSAGE+="${ANSI_WHITE}A compatible Linux distribution (Debian family) has NOT been detected!${ANSI_OFF}";
	echo -e "\n$MESSAGE\n";
	exit 1;
fi;
#
...
sudo apt-get install -y openssh-server coreutils sudo nano;
#
...
#sudo apt-get install -y open-vm-tools exfat-fuse;
#
...
sudo apt-get install -y libstdc++6 libstdc++6:i386 lib32gcc-s1 ncompress bzip2;
#
#	Required by scripts:
#
...
#
sudo dpkg -l | grep webmin 2> /dev/null > /dev/null;
if [[ "$?" -gt 0 ]]; then
		WEBMIN_CHECK=false;
	else
		WEBMIN_CHECK=true
...
if [ $WEBMIN_CHECK == true -a $TEST_USER_EXISTS == true ]; then
		...
		case $PROMPT_INPUT in
			y|Y|yes|Yes|YES|nike)
				...
				chown -R game-servers:game-serves $FILEMIN_BOOKMARKS_FOLDER 2> /dev/null > /dev/null;
				#chown game-servers:game-serves $FILEMIN_BOOKMARKS_FILE 2> /dev/null > /dev/null;
				#
